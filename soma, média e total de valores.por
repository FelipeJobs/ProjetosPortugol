programa
{/* nesse exercício tenho que usar o enquanto
Elaborar um programa que efetue a leitura sucessiva de valores 
numéricos e apresente no final o total do somatório, a média e o 
total de valores lidos. O programa deve fazer as leituras dos 
valores enquanto o usuário estiver fornecendo valores positivos. 
Ou seja, o programa deve parar quando o usuário fornecer um valor 
negativo.
*/
	
	funcao inicio()
	{
		real num = 0.0,soma =0.0,media
		inteiro totalDeValores, cont = 0
		escreva("Digite um valor para iniciarmos a soma: ")
			leia(num)
		enquanto(num > 0.0){
			soma = soma + num
			cont = cont +1
			escreva("Digite outro valor para somarmos: ")
			leia(num)
		}
		media = soma/cont
		escreva("\nO total da soma foi: " + soma)
		escreva("\nVocê somou um total de: " + cont + " Valores.")
		escreva("\nA média dos valores digitados foi: " + media)
		
		
	}
}
/* $$$ Portugol Studio $$$ 
 * 
 * Esta seção do arquivo guarda informações do Portugol Studio.
 * Você pode apagá-la se estiver utilizando outro editor.
 * 
 * @POSICAO-CURSOR = 423; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = ;
 * @FILTRO-ARVORE-TIPOS-DE-DADO = inteiro, real, logico, cadeia, caracter, vazio;
 * @FILTRO-ARVORE-TIPOS-DE-SIMBOLO = variavel, vetor, matriz, funcao;
 */